...
{
    return GridView::create(nullptr, cocos2d::Size::ZERO);
}

GridView* GridView::create(TableViewDataSource* dataSource, cocos2d::Size const& size)
{
    return GridView::create(dataSource, size, nullptr);
}

GridView* GridView::create(TableViewDataSource* dataSource, cocos2d::Size const& size, Node *container)
{
...
bool GridView::initWithViewSize(cocos2d::Size const& size, Node* container/* = nullptr*/)
{
...
Vec2 GridView::_offsetFromIndex(ssize_t index)
{
    ...
    const cocos2d::Size cellSize = _dataSource->tableCellSizeForIndex(this, index);
    if (_vordering == VerticalFillOrder::TOP_DOWN)
    {
...
void GridView::_updateCellPositions()
{
    ...
    _vCellsPositions.resize(_cellsCount, cocos2d::Point(0, 0));
        ...
        switch (this->getDirection())
        {
            case Direction::HORIZONTAL:
            {
                ...
                for (int i = 0; i < _cellsCount; i++) {
                    ...
                    _vCellsPositions[i] = cocos2d::Point(currentPosX, currentPosY);
                }
                break;
            }
            default:
...
void GridView::onTouchEnded(Touch *pTouch, Event *pEvent)
{
    ...
    if (_touchedCell){
		cocos2d::Rect bb = this->getBoundingBox();
